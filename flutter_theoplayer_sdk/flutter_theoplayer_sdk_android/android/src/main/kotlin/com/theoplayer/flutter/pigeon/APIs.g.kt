...
interface THEOplayerNativeTHEOliveAPI {
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: THEOplayerNativeTHEOliveAPI?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.theoplayer_platform_interface.THEOplayerNativeTHEOliveAPI.switchToHesp$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { _, reply ->
            val wrapped: List<Any?> = try {
              api.switchToHesp()
  ...
  fun onSourceChange(sourceArg: SourceDescription?, callback: (Result<Unit>) -> Unit)
{
  ...
  fun onCurrentSourceChange(currentSourceArg: TypedSourcePigeon?, callback: (Result<Unit>) -> Unit)
{
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    val channelName = "dev.flutter.pigeon.theoplayer_platform_interface.THEOplayerFlutterAPI.onCurrentSourceChange$separatedMessageChannelSuffix"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
    channel.send(listOf(currentSourceArg)) {
      if (it is List<*>) {
        if (it.size > 1) {
          callback(Result.failure(FlutterError(it[0] as String, it[1] as String, it[2] as String?)))
        } else {
